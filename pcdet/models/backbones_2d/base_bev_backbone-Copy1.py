...
class UpBlock(nn.Module):
    def __init__(self, in_filters, out_filters, stride=1, kernel_size=(3, 3, 3), indice_key=None, up_key=None):
        ...
        self.trans_dilao = nn.Conv2d(in_filters, out_filters, kernel_size=(3,3), stride=stride, padding=1, bias=False, indice_key=indice_key + "new_up")
        self.trans_act = nn.LeakyReLU()
        self.trans_bn = nn.BatchNorm1d(out_filters)

        self.conv1 = nn.Conv2d(in_filters, out_filters, kernel_size=(1,3), stride=1, padding=(0,1), bias=False, indice_key=indice_key)
        self.act1 = nn.LeakyReLU()
        self.bn1 = nn.BatchNorm1d(out_filters)

        self.conv2 = nn.Conv2d(in_filters, out_filters, kernel_size=(3,1), stride=1, padding=(1,0), bias=False, indice_key=indice_key)
        self.act2 = nn.LeakyReLU()
        self.bn2 = nn.BatchNorm1d(out_filters)

        self.conv3 = nn.Conv2d(in_filters, out_filters, kernel_size=(3,3), stride=1, padding=1, bias=False, indice_key=indice_key)
        self.act3 = nn.LeakyReLU()
        self.bn3 = nn.BatchNorm1d(out_filters)
        # self.dropout3 = nn.Dropout3d(p=dropout_rate)

        self.up_subm = nn.ConvTranspose2d(out_filters, out_filters, 2, indice_key=up_key,
                                                  bias=False)
    ...
    def __init__(self, model_cfg, input_channels, cylind_range=None, cy_grid_size=None):
        ...
        for idx in range(num_levels):
            ...
            if len(upsample_strides) > 0:
                ...
                if stride >= 1:
                    self.deblocks.append(nn.Sequental(
